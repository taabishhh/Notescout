...
class Comments {
  ...
  function get($pid = 0) {
    ...
    $sql = "SELECT * FROM `hcomments` WHERE `post_id`=? ORDER BY `reply_id`";
    $this->stmt = $this->pdo->prepare($sql);
  ...
  //       $rid : reply id
    ...
    $sql = "INSERT INTO `hcomments` ($fields) VALUES ($values);";
  ...
  //       $message : comment message
    ...
    $sql = "UPDATE `hcomments` SET `name`=?, `message`=? WHERE `comment_id`=?;";
  ...
  // PARAM $cid : comment ID
    ...
    $this->start();
    $pass = $this->exec("DELETE FROM `hcomments` WHERE `comment_id`=?;", [$cid]);
    if ($pass) {
      $pass = $this->exec("DELETE FROM `hcomments` WHERE `reply_id`=?;", [$cid]);
    }
    $this->end($pass);
    return $pass;
  }
