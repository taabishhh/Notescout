...
class Comments {
  ...
  // ATTEMPT CONNECT
    ...
    try {
      $str = "mysql:host=" . DB_SERVER . ";charset=" . DB_CHARSET;
      if (defined('DB_NAME')) {
        $str .= ";dbname=" . DB_NAME;
      }
      $this->pdo = new PDO(
          $str, DB_USERNAME, DB_PASSWORD, [
  ...
  function get($pid = 0) {
    ...
    $sql = "SELECT * FROM `jcomments` WHERE `post_id`=? ORDER BY `reply_id`";
    $this->stmt = $this->pdo->prepare($sql);
  ...
  //       $rid : reply id
    ...
    $sql = "INSERT INTO `jcomments` ($fields) VALUES ($values);";
  ...
  //       $message : comment message
    ...
    $sql = "UPDATE `jcomments` SET `name`=?, `message`=? WHERE `comment_id`=?;";
  ...
  // PARAM $cid : comment ID
    ...
    $this->start();
    $pass = $this->exec("DELETE FROM `jcomments` WHERE `comment_id`=?;", [$cid]);
    if ($pass) {
      $pass = $this->exec("DELETE FROM `jcomments` WHERE `reply_id`=?;", [$cid]);
    }
    $this->end($pass);
    return $pass;
  }
